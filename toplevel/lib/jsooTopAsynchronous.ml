...
let (>>?) o f =
  ...
  o >>= function
  | Error (err, w) -> M.return (Error (err, w))
  | Success (x, w) ->
      f x >>= function
      | Error (err, w') -> M.return (Error (err, w @ w'))
      | Success (x, w') -> M.return (Success (x, w @ w'))

let return_success e = M.return (JsooTop.Wrapped.Success (e, []))
let return_unit_success = return_success ()
(* let return_error e = M.return (JsooTop.Wrapped.Error (e, [])) *)
...
(* let () = *)
  (* Location.register_error_ofLwt._exn *)
    (* (function *)
      (* | Js.Error e -> *)
...
type u =
    U : 'a msg_ty * 'a JsooTop.Wrapped.result M.t -> u
...
type toplevel = {
  ...
  mutable reset_worker: toplevel -> unit M.t;
  mutable after_init: toplevel -> unit M.t;
  pp_stdout: output;
  pp_stderr: output;
}
...
let onmessage worker (ev : _ Worker.messageEvent Js.t) =
  ...
  | ReturnSuccess (id, ty_v, v, w) -> begin
      try
        let U (ty_u, u) = IntMap.find id worker.wakeners in
        let Eq = check_equal ty_u ty_v in
        worker.wakeners <- IntMap.remove id worker.wakeners;
        M.fill u (JsooTop.Wrapped.Success (v, w));
        Js._false
      with
      | Not_found ->
        Firebug.console##warn
  ...
  | ReturnError (id, e, w) -> begin
      try
        let U (_, u) = IntMap.find id worker.wakeners in
        worker.wakeners <- IntMap.remove id worker.wakeners;
        M.fill u (JsooTop.Wrapped.Error (e, w));
        Js._false
      with Not_found ->
        Firebug.console##warn
          (Js.string (Printf.sprintf "Missing wakeners (%d)" id));
...
(** Threads created with [post] will always be wake-uped by
    [onmessage] by calling [M.wakeup]. They should never end with
    an exception, unless canceled. When canceled, the worker is
    killed and a new one is spawned. *)
let rec post : type a. toplevel -> a host_msg -> a JsooTop.Wrapped.result M.t =
  fun worker msg ->
    let msg_id = worker.counter in
    let msg_ty = ty_of_host_msg msg in
    let t = M.create () in
(*    M.on_cancel t
      (fun () -> M.async (fun () -> worker.reset_worker worker)); *)
    worker.wakeners <- IntMap.add msg_id (U (msg_ty, t)) worker.wakeners;
...
and do_reset_worker () =
  ...
  fun worker ->
    if !running then begin
      ...
      terminate worker;
      IntMap.iter
        (* GRGR: Peut-on 'cancel' directement le M.u ? *)
        (fun _ (U (_, t)) -> M.cancel t)
        worker.wakeners;
      ...
      let l = List.map (fun name ->
           import_cmis_js worker name >>= fun _ ->
           M.return ()) imported
      in
      List.fold_left (fun acc x -> x >>= fun () -> acc)
        (M.return ()) l >>= fun () ->
...
let reset worker ?(timeout = fun () -> M.create ()) () =
  let timeout = timeout () in
  ( post worker Reset >>= fun res -> M.return (`Reset res) )
  >>= function
